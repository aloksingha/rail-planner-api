...
import { handlePartialBookingCancellation } from './commission';

export async function cancelPassengersOrBooking(
    tx: Omit<Prisma.TransactionClient, '$use'>,
    bookingId: string,
    passengerNameToCancel?: string
) {
    ...
    if (booking.status === 'CANCELLED') throw new Error('Booking is already fully cancelled');
    ...
    await handlePartialBookingCancellation(tx, bookingId, refundRatio);
