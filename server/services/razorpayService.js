...
exports.parseWithdrawalDetails = parseWithdrawalDetails;
exports.processPayout = processPayout;
exports.processTicketRefund = processTicketRefund;
...
async function callRazorpayAPI(endpoint, method, body) {
...
async function processPayout(requestId) {
    console.log(`[Razorpay Payout] Starting processing for Request ID: ${requestId}`);
    const request = await prisma_1.prisma.withdrawalRequest.findUnique({
        where: { id: requestId },
        include: { user: true }
    });
    if (!request) {
...
async function processTicketRefund(refundId) {
    console.log(`[Razorpay Refund] Starting processing for Refund ID: ${refundId}`);
    const refund = await prisma_1.prisma.refundRecord.findUnique({
        where: { id: refundId },
        include: { user: true }
    });
    if (!refund) {
    ...
    if (refund.paymentId.startsWith('WAL_')) {
        ...
        try {
            ...
            await prisma_1.prisma.refundRecord.update({
                where: { id: refundId },
                data: {
                    status: 'FAILED',
