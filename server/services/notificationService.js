...
exports.notifyPaymentReceived = exports.notifyBookingCancelled = exports.notifyBookingConfirmed = exports.sendWhatsApp = exports.sendSMS = void 0;
 ...
 */
const sendSMS = async (mobile, templateId, params, dltTemplateId) => {
    ...
    try {
        ...
        const response = await axios_1.default.post('https://infozysms.in/api/sms/send', payload, {
            headers: {
 ...
 */
const sendWhatsApp = async (mobile, templateId, params) => {
...
// ─── Booking Confirmed ────────────────────────────────────────────────────────
const notifyBookingConfirmed = async (email, eventName, mobile) => {
...
<body style="margin:0;padding:0;background:#0f172a;font-family:'Segoe UI',sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f172a;padding:40px 20px;">
    ...
    try {
        ...
        if (mobile && template) {
            await (0, exports.sendSMS)(mobile, template, {
...
// ─── Booking Cancelled ────────────────────────────────────────────────────────
const notifyBookingCancelled = async (email, reason, mobile) => {
    // 1. Send Email (non-blocking Promise)
    transporter.sendMail({
        from: FROM,
        to: email,
...
const notifyPaymentReceived = async (mobile, amount, orderId) => {
