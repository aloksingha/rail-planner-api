...
exports.getTicketPrice = exports.extractCode = void 0;
...
const resolveToCode = (str, mappings = []) => {
    const raw = (0, exports.extractCode)(str);
    if (!raw)
        return '';
    if (raw.length <= 4 && /^[A-Z0-9]+$/.test(raw))
        return raw;
...
const getTicketPrice = (srcRaw, dstRaw, clsRaw, tName, tTravelTime, context) => {
    if (!context) {
    ...
    const custom = customPrices.find(p => (0, exports.extractCode)(p.source) === src &&
        (0, exports.extractCode)(p.destination) === dst &&
        p.class === cls);
    ...
    for (const corridor of corridors) {
        try {
            const origins = JSON.parse(corridor.originStations || '[]').map((s) => resolveToCode(String(s), mappings));
            const destinations = JSON.parse(corridor.destinationStations || '[]').map((s) => resolveToCode(String(s), mappings));
    ...
    for (const sc of activeCharges) {
        if (cls === 'SL')
            extraCharge += sc.amountSL;
        else if (cls === '3A' || cls === '3E' || cls === 'CC')
            extraCharge += sc.amount3A;
        else if (cls === '2S')
            extraCharge += sc.amount2S;
        else if (cls === '2A' || cls === 'FC')
            extraCharge += sc.amount2A;
        else if (cls === '1A' || cls === 'EC' || cls === 'EV')
            extraCharge += sc.amount2A;
    }
