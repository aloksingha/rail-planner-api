...
router.get('/search', async (req, res) => {
    try {
        const { pick_up_latitude = '40.6397018432617', pick_up_longitude = '-73.7791976928711', drop_off_latitude = '40.6397018432617', drop_off_longitude = '-73.7791976928711', pick_up_date, drop_off_date, pick_up_time = '10:00', drop_off_time = '10:00', driver_age = '30', currency_code = 'USD', location = 'US', } = req.query;
        ...
        const response = await axios_1.default.get(`https://booking-com15.p.rapidapi.com/api/v1/cars/searchCarRentals?${params.toString()}`, { headers: BOOKING_HEADERS, timeout: 15000 });
        ...
        // Adapt results
        const results = raw?.search_results || raw?.results || (Array.isArray(raw) ? raw : []);
        ...
        }).filter((c) => c.totalPrice > 0 || c.basePrice > 0);
