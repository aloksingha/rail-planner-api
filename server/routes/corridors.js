...
router.post('/seed', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ADMIN']), async (req, res) => {
    try {
        const defaults = [
            {
        ...
        for (const d of defaults) {
            const exists = await prisma_1.prisma.corridorPricing.findFirst({ where: { name: d.name } });
            if (!exists) {
                await prisma_1.prisma.corridorPricing.create({ data: d });
                createdCount++;
            }
        }
...
router.get('/public', async (req, res) => {
    try {
        const corridors = await prisma_1.prisma.corridorPricing.findMany({
            orderBy: { name: 'asc' }
        });
...
router.post('/', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ADMIN']), async (req, res) => {
    ...
    try {
        const corridor = await prisma_1.prisma.corridorPricing.create({
            data: {
                name,
...
router.put('/:id', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ADMIN']), async (req, res) => {
    const { id } = req.params;
    const { name, originStations, destinationStations, markupSL, markup3A, markup2A } = req.body;
    try {
        const corridor = await prisma_1.prisma.corridorPricing.update({
            where: { id: id },
            data: {
                name,
...
router.delete('/:id', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ADMIN']), async (req, res) => {
    const { id } = req.params;
    try {
        const corridor = await prisma_1.prisma.corridorPricing.delete({
            where: { id: id }
        });
        await (0, auditService_1.createAuditLog)({
