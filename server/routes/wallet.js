...
async function getWithdrawableBalance(userId, currentBalance) {
    const pendingCommissionsTx = await prisma_1.prisma.walletTransaction.findMany({
        where: {
            userId,
            type: 'CREDIT',
    ...
    if (pendingCommissionsTx.length > 0) {
        const bookingIds = pendingCommissionsTx.map(t => t.bookingId);
        ...
        pendingCommissionsTx.forEach(t => {
            const status = bookingStatusMap.get(t.bookingId);
 ...
 */
router.get('/history', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ADMIN', 'SALES_MANAGER', 'CUSTOMER']), async (req, res) => {
 ...
 */
router.post('/admin/adjust', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN']), async (req, res) => {
 ...
 */
router.get('/admin/all-transactions', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN']), async (req, res) => {
    try {
 ...
 */
router.post('/withdraw-request', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ADMIN', 'SALES_MANAGER', 'CUSTOMER']), async (req, res) => {
    ...
    let parsedDetails;
    try {
        parsedDetails = (0, razorpayService_1.parseWithdrawalDetails)(details);
    }
    catch (err) {
    ...
    try {
        ...
        (0, razorpayService_1.processPayout)(result.id).catch((err) => {
 ...
 */
router.get('/my-withdrawals', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ADMIN', 'SALES_MANAGER', 'CUSTOMER']), async (req, res) => {
    try {
 ...
 */
router.get('/admin/withdrawals', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN']), async (req, res) => {
    try {
        const withdrawals = await prisma_1.prisma.withdrawalRequest.findMany({
            include: {
 ...
 */
router.patch('/admin/process-withdrawal/:id', auth_1.requireAuth, (0, auth_1.requireRole)(['SUPER_ADMIN']), async (req, res) => {
