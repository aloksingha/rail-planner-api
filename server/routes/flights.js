...
function generateSmartMock(srcCode, dstCode, flightDate, currency, cabinClass) {
    ...
    const classMulti = { economy: 1, business: 3.2, first: 5.5 };
    const cm = classMulti[cabinClass.toLowerCase()] || 1;
    const baseFareINR = Math.round(((distKm * 4.8 + 800) * cm) / 50) * 50;
    const currencyRates = { INR: 1, USD: 0.012, EUR: 0.011, GBP: 0.0095, AED: 0.044 };
...
router.get('/search', async (req, res) => {
    try {
        const { sourceCode = 'DEL', destCode = 'BOM', date, returnDate, tripType = 'oneway', adults = '1', cabinClass = 'economy', currency = 'INR', directOnly = 'false', sortBy = 'best', } = req.query;
        ...
        try {
            ...
            const response = await axios_1.default.get(url, { timeout: 25000 });
        ...
        if (results.length === 0) {
            ...
            if (srcInfo && dstInfo) {
                try {
                    ...
                    const response = await axios_1.default.get(`https://sky-scrapper.p.rapidapi.com/api/v1/flights/searchFlights?${apiParams.toString()}`, { headers: SKYSCRAPPER_HEADERS, timeout: 25000 });
        ...
        if (sortBy === 'price')
            ...
            results.sort((a, b) => (a.durationMinutes || 9999) - (b.durationMinutes || 9999));
